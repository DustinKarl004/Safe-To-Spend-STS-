  ...
  <Transition name="slide">
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4">
        <div v-for="group in EXPENSE_GROUPS" :key="group.kind" class="mb-5 last:mb-0">
          <h3 class="mb-2.5 text-xs font-semibold uppercase tracking-wide text-text-dim">{{ group.label }}</h3>
        ...
        <p v-if="!wallets.length" class="rounded-xl border border-dashed border-border p-4 text-sm text-text-dim">
          Add a wallet first before logging an expense.
        </p>
          ...
          >
            ...
            <span class="shrink-0 text-xs font-semibold text-text-dim">
              <template v-if="walletId === wallet.id && Number(amountStr) > 0">
                {{ formatPeso(wallet.balance) }} → <span class="text-danger">{{ formatPeso(projectedBalance) }}</span>
              </template>
